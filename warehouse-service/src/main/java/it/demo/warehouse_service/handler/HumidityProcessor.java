...
@Service
public class HumidityProcessor implements SensorProcessor {

    Logger log = LoggerFactory.getLogger(HumidityProcessor.class);

    private final SensorProperty sensorProperty;

    private final SensorInformationProducer sensorInformationProducer;

    public HumidityProcessor(SensorInformationProducer sensorInformationProducer, SensorsProperties sensorsProperties) {
    ...
    public Mono<Void> handleMessage(String message) {
        ...
        if (!matcher.matches()) {
            log.warn("Invalid humidity sensor message format: {}",  message);
            return Mono.empty();
        }
